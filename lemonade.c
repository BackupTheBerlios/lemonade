...
**
**  Dokumentation: http://www.hwaci.com/sw/lemon/lemon.html
**
*/
#include <stdio.h>
#include <stdarg.h>
...
struct lemon *lemp;
{
  static char templatename[] = "lempar.d";
  char buf[1000];
  FILE *in;
  char *tpltname;
  char *cp;
...
//  fprintf(out,"#define %sTOKENTYPE %s\n",name,lemp->tokentype?lemp->tokentype:"void*");  lineno++;
  fprintf(out,"alias %s %sTOKENTYPE; \n",lemp->tokentype?lemp->tokentype:"void*",name);  lineno++;
  fprintf(out,"alias %s GTOKENTYPE; \n",lemp->tokentype?lemp->tokentype:"void*");  lineno++;
...
//  fprintf(out,"#define YYNOCODE %d\n",lemp->nsymbol+1);
  fprintf(out,"const int YYNOCODE = %d;\n",lemp->nsymbol+1);
  lineno++;
...
//    fprintf(out,"#define YYSTACKDEPTH %s\n",lemp->stacksize);  lineno++;
    fprintf(out,"const int YYSTACKDEPTH = %s;\n",lemp->stacksize);  lineno++;
  }else{
//    fprintf(out,"#define YYSTACKDEPTH 100\n");  lineno++;
    fprintf(out,"const int YYSTACKDEPTH = 100;\n");  lineno++;
  }
//  fprintf(out, "#endif\n"); lineno++;
  fprintf(out, "\n"); lineno++;
  if( mhflag ){
//    fprintf(out,"#if INTERFACE\n"); lineno++;
    fprintf(out,"static if INTERFACE\n"); lineno++;
  }
  name = lemp->name ? lemp->name : "Parse";
...
//    fprintf(out,"#define %sARG_SDECL %s;\n",name,lemp->arg);  lineno++;
     fprintf(out," struct %sARG_SDECL { %s; }\n",name,lemp->arg);  lineno++;
...
//    fprintf(out,"#define %sARG_SDECL\n",name);  lineno++;
    fprintf(out," struct  %sARG_SDECL { }\n",name);  lineno++;
//    fprintf(out,"#define %sARG_PDECL\n",name);  lineno++;
//    fprintf(out,"const int %sARG_PDECL = 0;\n",name);  lineno++;
...
//    fprintf(out,"#endif\n"); lineno++;
  ...
  fprintf(out,"const char[]  YYERRSYMDT = \"yy%d\";\n",lemp->errsym->dtnum);  lineno++;
  if( lemp->has_fallback ){
    fprintf(out,"const int  YYFALLBACK = 1;\n");  lineno++;
  }
  else
  {
    fprintf(out,"const int  YYFALLBACK = 0;\n");  lineno++;
  }
  tplt_xfer(lemp->name,in,out,&lineno);
...
struct lemon *lemp;
{
  ...
  if( lemp->tokenprefix ) prefix = lemp->tokenprefix;
    ...
    in = file_open(lemp,".d","rb");
  if( in ){
  	sprintf(pattern,"\n\n\n/***** Tokens *********/\n");
    for(i=1; i<lemp->nterminal && fgets(line,LINESIZE,in); i++){
      sprintf(pattern,"const int %s%-30s = %2d;\n",prefix,lemp->symbols[i]->name,i);
